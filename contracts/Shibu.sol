...
contract Shibu is ERC20, Ownable {
    ...
    constructor()  ERC20("SHIBU", "SHIBU") {
        ...
        address _pair = IFactory(_router.factory())
            .createPair(address(this), _router.WBNB());
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SHIBU: The dividend tracker already has that address");

        ShibuDividendTracker newDividendTracker = ShibuDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SHIBU: The new dividend tracker must be owned by the SHIBU token contract");
    ...
    function updateRouter(address newAddress) public onlyOwner {
        require(newAddress != address(router), "SHIBU: The router already has that address");
        emit Updaterouter(newAddress, address(router));
        router = IRouter(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SHIBU: Account is already the value of 'excluded'");
    ...
    function _setAutomatedMarketMakerPair(address newPair, bool value) private {
        require(automatedMarketMakerPairs[newPair] != value, "SHIBU: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SHIBU: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SHIBU: Cannot update gasForProcessing to same value");
